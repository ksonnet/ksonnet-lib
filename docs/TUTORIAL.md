...
This tutorial demonstrates the power of custom **ksonnet** mixins. It walks through example code that we've published on GitHub, at mixins/incubator/fluentd. These examples provide a set of **ksonnet** 
libraries and files that define configurations to add Fluentd log collection and aggregation, Elasticsearch log analytics, and a Kibana dashboard to your cluster.

For more information about Elasticsearch and Kibana, see [the Elastic website](https://www.elastic.co/products). For Fluentd, see [the Fluentd website](http://www.fluentd.org/architecture).

## Requirements to build your own mixins

If you want to build your own mixins, or write **ksonnet** using the built-in mixins, you need to perform the following tasks. For details, see the [readme][readme].

* Install **Jsonnet**, version 0.9.4 or later
* Clone the **ksonnet** repository locally
* Install and configure the VisualStudio Code extension 
(optional)
* Create a test Kubernetes cluster

## Architecture and design

We'll walk through the key parts of the files in the GitHub mixins repository
in detail, but at a high level this tutorial demonstrates the following aspects of the larger example set:

* A basic DaemonSet that defines Fluentd. On its own, this DaemonSet can't 
do anything. It has no permissions to access pod logs, nor does it have volumes attached.
* A separate mixin that defines the VolumeMounts and Volumes that the DaemonSet requires
* A separate mixin that configures the access permissions for the DaemonSet
* The RBAC objects that must be associated with the Service Account for the cluster so that the access permissions can be appropriately applied
...
Note particularly in the following snippet the `containerSelector` parameter to `addHostMountedPodLogs`. We pass this function to `ds.mapContainers` to iterate 
over our containers (in this case, our Fluentd containers) and add the VolumeMounts that they need. (The details of the pod logs have also been abstracted away to their own function.)
...
The relevant fields here are `namespace` and `accountName`, which we pass as the 
arguments that our RBAC snippet needs when it calls the `rbac` function.

## Wrap it all up

Here's where we started, with our simple DaemonSet, its pod logging, and its access 
permissions. But now you've seen what's going on underneath -- not just how the 
functions for adding pod logs and permissions are clearly separated, but how 
we can customize them as needed without having to rewrite the entire configuration.
...
The GitHub example directory also includes the generated JSON files. Examine them to 
help understand the details of how **ksonnet**'s decomposition and abstraction are 
compiled into complete configurations.

As you start to write your own custom mixins, look also at how we break down the 
basic **ksonnet** imports into smaller component objects for easier manipulation.
